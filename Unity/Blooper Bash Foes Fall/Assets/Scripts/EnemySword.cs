...
public class EnemySword : MonoBehaviour {
	...
	void FixedUpdate() {
		if (!player.GetComponent<swordEnemy>().isAttacking) return;
		...
		if (transform.position.x>other.gameObject.transform.position.x)enemyDirection=new Vector2(-50, 10);
		else enemyDirection=new Vector2(50, 10);
		other.gameObject.GetComponent<Rigidbody2D>().velocity=enemyDirection;
		PlayerMovement toAttack=other.gameObject.GetComponent<PlayerMovement>();
		toAttack.health-=1;
